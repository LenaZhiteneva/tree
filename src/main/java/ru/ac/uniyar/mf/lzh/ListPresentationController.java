...
public class ListPresentationController {
    ...
    public String getTree() {
        String result = "<html>" +
                ...
                "  </body>" +
                "</html>";
        result += "       <form method=\"post\" action=\"add_note\">" +
    ...
    @Path("/edit_node/{id}")
    @Produces("text/html")
    public String getEditNode(@PathParam("id") int itemId) {
        Node node = root;
        String result =
                "<html>" +
                        "  <head>" +
                        ...
                        "    <form method=\"post\" action=\"/edit_node/" + itemId + "\">" +
                        "      <p>Значение</p>" +
    ...
    @Path("/edit_node/{id}")
    @Produces("text/html")
    public Response editNode(@PathParam("id") String itemId, @FormParam("value") String itemValue) {
        Scanner in = new Scanner(System.in);
        String str = in.next();
        root.find_with_id(itemId).change_name("str");
        try {
            return Response.seeOther(new URI("tree")).build();
        } catch (URISyntaxException e) {
            throw new IllegalStateException("Ошибка построения URI для перенаправления");
